...
# PACKET STRUCTURE UPDATED: Added 3x uint16_t (3H) for the FSRs before the thermal grid
# uint32 (I), uint8 (B), 6 floats (6f), 3 uint16 (3H), 64 floats (64f)
# Total packet size is now exactly 291 bytes.
...
fig.canvas.manager.set_window_title('Landing Capsule GCS')
fig.suptitle('Landing Capsule COMMAND & CONTROL', fontsize=18, fontweight='bold', color='cyan', y=0.95)
...
print("\n======================================")
print("[*] GCS Online. FSR-Triple Array Active.")
    ...
    while True:
        ...
        if current_time - last_debug_print > 2.0:
            if packets_received == 0:
                ...
                print(f"[STATUS] Telemetry Link Active. (FSR Array: {last_fsr1}, {last_fsr2}, {last_fsr3})")
                packets_received = 0
        ...
        if latest_data and len(latest_data) == PACKET_SIZE:
            ...
            if previous_state == 4 and state_id >= 5:
                if len(impact_t_data) > 1:
                    ...
                    valid_peaks = [p for p in peaks if g_arr[p] > 0.5] 
                    ...
                    # FSR ASYMMETRIC LANDING CHECK
                    # If the max raw FSR value differs from the min raw FSR by > 250 logic points,
                    # it means one side took the hit while the other remained unloaded.
                    fsr_vals = [last_fsr1, last_fsr2, last_fsr3]
                    max_fsr = max(fsr_vals)
                    min_fsr = min(fsr_vals)
                    
                    if (max_fsr - min_fsr) > 250:
                        print(f"\n[!] WARNING: ASYMMETRIC LANDING DETECTED [!]")
                        print(f"    Resting Load Spread: {fsr_vals}")
                        print(f"    Capsule is resting at a severe angle.")
                    else:
                        print(f"\n[OK] Symmetric Landing Confirmed.")
                        print(f"    Resting Load Spread: {fsr_vals}")
