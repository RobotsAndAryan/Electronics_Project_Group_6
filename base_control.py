...
# 2 Rows, 3 Columns. Top row: HUDs + Data. Bottom row: Wide Graph.
...
# Pitch ladder lines for "cool" factor
for p in [-60, -40, -20, 20, 40, 60]:
    ax_att.plot([-15, 15], [p, p], color='gray', linewidth=1)
circle = plt.Circle((0, 0), 45, color='green', fill=False, linestyle='--', linewidth=2)
...
ax_graph.set_ylabel("Magnitude (G)", color='white')
...
# --- 5. LAUNCH BUTTON (ABSOLUTE POSITIONING) ---
    ...
    while True:
        ...
        if latest_data and len(latest_data) == PACKET_SIZE:
            ...
            if state_id == 4:
                ...
                current_g = np.sqrt(ax**2 + ay**2 + az**2) / 9.81
                
                impact_t_data.append(rel_t)
                impact_g_data.append(current_g)
                line_gforce.set_data(impact_t_data, impact_g_data)
                
                if current_g > ax_graph.get_ylim()[1]:
                    ax_graph.set_ylim(0, current_g + 2.0)
            ...
            if previous_state == 4 and state_id >= 5:
                if len(impact_t_data) > 1:
                    ...
                    valid_peaks = [p for p in peaks if g_arr[p] > 1.1]
                    
                    damping_ratio = None
                    if len(valid_peaks) >= 2:
                        x1 = g_arr[valid_peaks[0]] - 1.0 
                        x2 = g_arr[valid_peaks[1]] - 1.0
                        if x1 > x2 > 0:
                            delta = np.log(x1 / x2)
